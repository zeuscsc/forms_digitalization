...
async function callLiteLLM(fileBuffer: Buffer, mimeType: string = "application/pdf"): Promise<string> {
  ...
  const dataUri = `data:${mimeType};base64,${base64Pdf}`;
...
export async function POST(request: NextRequest) {
  try {
    ...
    const pdfPath = path.join(uploadDir, `uploaded-${timestamp}.${ext}`);
    const relativePdfPath = path.posix.join(UPLOADED_FORMS_DIR_NAME, `uploaded-${timestamp}.${ext}`);
    ...
    console.log(`Executing LiteLLM for ${slug} with type ${file.type}`);

    const generatedJsonString = await callLiteLLM(buffer, file.type || "application/pdf");
